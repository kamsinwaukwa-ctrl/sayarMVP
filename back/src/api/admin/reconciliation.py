...
async def get_reconciliation_status(
    ...
    try:
        ...
        query = (
            ...
            .order_by(MetaReconciliationRun.started_at.desc())
            .limit(1)
        )
        ...
        if not latest_run:
            return ApiResponse(
                ...
                data=ReconciliationStatusResponse(
                    last_run_at=None, status=None, stats=None
                ),
        ...
        return ApiResponse(
            ok=True,
            data=ReconciliationStatusResponse(
                last_run_at=run.started_at, status=run.status, stats=run.stats
            ),
...
async def trigger_reconciliation(
    merchant_id: Optional[UUID] = Query(
        None, description="Merchant ID to reconcile (if not provided, reconciles all)"
    ),
    ...
    try:
        ...
        if merchant_id:
            ...
            increment_counter(
                "meta_reconciliation_manual_triggers_total", {"scope": "merchant"}
            )

            return ApiResponse(
                ok=True,
                data=TriggerReconciliationResponse(
                    job_id=run_id, scheduled_at=datetime.utcnow()
                ),
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=create_error_response(
                "RECONCILIATION_TRIGGER_ERROR", "Failed to trigger reconciliation"
            ).dict(),
        )
    ...
    try:
        ...
        return ApiResponse(
            ...
            data=ReconciliationHistoryResponse(
                runs=runs, total=total, pagination=pagination
            ),
...
async def get_merchant_reconciliation_status(
    ...
    try:
        ...
        logger.error(
            ...
            extra={"merchant_id": str(user.merchant_id), "error": str(e)},
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
