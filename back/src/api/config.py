...
async def create_system_setting(
    ...
    try:
        ...
        return APIResponse(data=result, message="System setting created successfully")
...
async def list_system_settings(
    ...
    return APIResponse(
        data=settings, message=f"Retrieved {len(settings)} system settings"
    )
...
async def get_system_setting(
    ...
    return APIResponse(data=setting, message="System setting retrieved successfully")
...
async def update_system_setting(
    ...
    try:
        result = await service.update_system_setting(
            key, update_data, current_user.role
        )

        # Invalidate cache
        invalidate_config_cache()

        return APIResponse(data=result, message="System setting updated successfully")
...
async def delete_system_setting(
    ...
    try:
        ...
        return APIResponse(data=True, message="System setting deleted successfully")
...
async def create_merchant_setting(
    ...
    try:
        result = await service.create_merchant_setting(
            current_user.merchant_id, setting, current_user.role
        )

        # Invalidate cache for this merchant
        invalidate_config_cache(current_user.merchant_id)

        return APIResponse(data=result, message="Merchant setting created successfully")
...
async def list_merchant_settings(
    ...
    return APIResponse(
        data=settings, message=f"Retrieved {len(settings)} merchant settings"
    )
...
async def get_merchant_setting(
    ...
    return APIResponse(data=setting, message="Merchant setting retrieved successfully")
...
async def update_merchant_setting(
    ...
    try:
        result = await service.update_merchant_setting(
            current_user.merchant_id, key, update_data, current_user.role
        )

        # Invalidate cache for this merchant
        invalidate_config_cache(current_user.merchant_id)

        return APIResponse(data=result, message="Merchant setting updated successfully")
...
async def delete_merchant_setting(
    ...
    try:
        await service.delete_merchant_setting(
            current_user.merchant_id, key, current_user.role
        )

        # Invalidate cache for this merchant
        invalidate_config_cache(current_user.merchant_id)

        return APIResponse(data=True, message="Merchant setting deleted successfully")
...
async def create_feature_flag(
    ...
    try:
        ...
        return APIResponse(data=result, message="Feature flag created successfully")
...
async def list_feature_flags(
    ...
    flags = await service.list_feature_flags(
        merchant_id=current_user.merchant_id, include_global=include_global, query=query
    )

    return APIResponse(data=flags, message=f"Retrieved {len(flags)} feature flags")
...
async def get_feature_flag(
    name: str,
    global_flag: bool = Query(
        False, description="Get global flag instead of merchant-specific"
    ),
    current_user: User = Depends(require_auth),
    db: AsyncSession = Depends(get_db),
) -> APIResponse[FeatureFlagResponse]:
    ...
    return APIResponse(data=flag, message="Feature flag retrieved successfully")
...
async def update_feature_flag(
    ...
    global_flag: bool = Query(
        False, description="Update global flag instead of merchant-specific"
    ),
    current_user: User = Depends(require_auth),
    db: AsyncSession = Depends(get_db),
) -> APIResponse[FeatureFlagResponse]:
    ...
    try:
        result = await service.update_feature_flag(
            name, merchant_id, update_data, current_user.role
        )
        ...
        return APIResponse(data=result, message="Feature flag updated successfully")
...
async def delete_feature_flag(
    name: str,
    global_flag: bool = Query(
        False, description="Delete global flag instead of merchant-specific"
    ),
    current_user: User = Depends(require_auth),
    db: AsyncSession = Depends(get_db),
) -> APIResponse[bool]:
    ...
    try:
        ...
        return APIResponse(data=True, message="Feature flag deleted successfully")
...
async def get_merged_configuration(
    ...
    return APIResponse(
        data=merged_config, message="Merged configuration retrieved successfully"
    )
...
async def bulk_update_configuration(
    ...
    try:
        result = await service.bulk_update_configuration(
            current_user.merchant_id, bulk_update, current_user.role
        )
        ...
        return APIResponse(
            data=result, message="Bulk configuration update completed successfully"
        )
...
async def invalidate_configuration_cache(
    merchant_id: Optional[UUID] = Query(
        None, description="Specific merchant ID to invalidate"
    ),
    current_user: User = Depends(require_admin),
    db: AsyncSession = Depends(get_db),
) -> APIResponse[bool]:
    ...
    return APIResponse(
        data=True, message=f"Configuration cache invalidated for {scope}"
    )
