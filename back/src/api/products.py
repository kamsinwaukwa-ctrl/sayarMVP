...
async def list_products(
    ...
    meta_sync_status: Optional[str] = Query(
        None, description="Filter by Meta sync status"
    ),
    meta_catalog_visible: Optional[bool] = Query(
    ...
    try:
        ...
        products, total_count = await service.list_products(
            merchant_id=principal.merchant_id, filters=filters, pagination=pagination
        )
        ...
        return ApiResponse(
            data=response_data, message=f"Retrieved {len(products)} products"
        )
...
async def get_product(
    ...
    try:
        ...
        product = await service.get_product(
            product_id=product_id, merchant_id=principal.merchant_id
        )
        ...
        return ApiResponse(
            data=product.model_dump(), message="Product retrieved successfully"
        )
...
@router.delete(
    ...
    responses={
        ...
        400: {
            ...
            "description": "Cannot delete product with reservations",
        },
    },
...
async def trigger_meta_sync(
    ...
    try:
        ...
        job_id = await service.enqueue_manual_catalog_sync(
            product_id=product_id, merchant_id=admin.merchant_id, requested_by=admin.id
        )

        # Increment success metric
        increment_counter(
            "sayar_manual_sync_requests_total",
            tags={"merchant_id": str(admin.merchant_id), "status": "success"},
        )

        response_data = MetaSyncResponse(
            product_id=product_id, sync_status="pending", job_id=job_id
        )
        ...
        if "already in progress" in error_msg:
            increment_counter(
                "sayar_manual_sync_conflicts_total",
                tags={"merchant_id": str(admin.merchant_id)},
            )
            raise HTTPException(
        ...
        increment_counter(
            "sayar_manual_sync_requests_total",
            tags={"merchant_id": str(admin.merchant_id), "status": "error"},
        )

        logger.error(
            f"Failed to trigger manual sync: {str(e)}",
            extra={
...
async def get_meta_sync_status(
    product_id: UUID, user=Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        ...
        stmt = select(Product).where(
            Product.id == product_id, Product.merchant_id == user.merchant_id
        )
        ...
        if not sync_reason and sync_status == "error" and sync_errors:
            ...
            if sync_reason:
                try:
                    ...
                    logger.warning(
                        f"Failed to backfill sync reason: {str(e)}",
                        extra={
                            "product_id": str(product_id),
        ...
        # Increment metrics
        increment_counter(
            "meta_sync_status_requests_total",
            tags={"merchant_id": str(user.merchant_id), "status": sync_status},
        )

        if sync_reason:
            reason_category = MetaSyncReasonNormalizer.get_reason_category(sync_reason)
            increment_counter(
                "meta_sync_reason_category_total", tags={"category": reason_category}
            )
        ...
        logger.error(
            f"Failed to get Meta sync status: {str(e)}",
            extra={
                "merchant_id": str(user.merchant_id),
                "product_id": str(product_id),
                "error": str(e),
...
@router.post(
    ...
    responses={
        202: {
            "model": ApiResponse,
            "description": "Unpublish job enqueued successfully",
        },
...
async def force_unpublish_product(
    ...
    try:
        ...
        response_data = MetaUnpublishResponse(
            product_id=product_id, action="force_unpublish", job_id=job_id
        )
        ...
        error_message = str(e)
        logger.warning(
            f"Force unpublish validation error: {error_message}",
            extra={
                "merchant_id": str(admin.merchant_id),
                "product_id": str(product_id),
                "error": error_message,
        ...
        logger.error(
            f"Failed to force unpublish product: {str(e)}",
            extra={
                "merchant_id": str(admin.merchant_id),
                "product_id": str(product_id),
                "error": str(e),
