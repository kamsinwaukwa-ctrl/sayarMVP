...
async def get_my_rate_limits(
    ...
    return APIResponse(ok=True, data=MerchantRateLimitConfig(**row._asdict()))
...
async def reset_merchant_rate_limits(
    ...
    logger.info("rate_limit_reset", extra={"merchant_id": merchant_id})

    return APIResponse(ok=True, message="Rate limit counters reset successfully")
