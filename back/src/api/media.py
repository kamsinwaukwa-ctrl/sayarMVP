...
async def upload_logo(
    file: Annotated[UploadFile, File(description="Logo image file (PNG, JPEG, or WebP)")],
    ...
    """
    logger.info(f"Logo upload request from user {current_user.user_id} for merchant {current_user.merchant_id}")
    increment_counter("media_upload_requests_total", {
...
async def get_logo_signed_url(
    ...
    """
    logger.info(f"Signed URL request from user {current_user.user_id} for merchant {current_user.merchant_id}")
    increment_counter("signed_url_requests_total", {
        "merchant_id": str(current_user.merchant_id),
        "user_id": str(current_user.user_id),
...
async def delete_logo(
    ...
    """
    logger.info(f"Logo deletion request from user {current_user.user_id} for merchant {current_user.merchant_id}")
    increment_counter("media_deletion_requests_total", {
        "merchant_id": str(current_user.merchant_id),
        "user_id": str(current_user.user_id)
    })
...
async def media_health_check(
    ...
    try:
        ...
        return APIResponse(
            ok=health_status["status"] != "unhealthy",
            data=health_status
        )
