...
from ..models.auth import RegisterRequest, LoginRequest, RegisterResponse, AuthResponse, UserResponse
from ..models.api import ApiResponse, ApiErrorResponse
from ..database.connection import get_db
from ..services.auth_service import AuthService, AuthError
from ..dependencies.auth import get_current_user, CurrentUser
from ..middleware.rate_limit import (
    check_login_rate_limit, check_email_rate_limit, record_login_attempt, get_rate_limit_headers
)
...
async def login_user(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"JWT creation failed: {str(jwt_error)}")
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Response building failed: {str(response_error)}")
...
async def debug_register_test(request: RegisterRequest):
    ...
    try:
        ...
        return {"status": "success", "email": request.email, "business_name": request.business_name}
    except Exception as e:
...
async def debug_db_connection(db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    try:
        ...
        return {"db_test": row.test if row else None, "functions_found": functions_found}
