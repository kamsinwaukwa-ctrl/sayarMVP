...
async def get_merchant_feed(
    ...
    **Public Endpoint** - No authentication required (consumed by Meta)
    ...
    # Log request
    logger.info("meta_feed_requested", extra={
    ...
    try:
        ...
        if not check_rate_limit(client_ip):
            ...
            logger.warning("meta_feed_rate_limited", extra={
                "merchant_slug": merchant_slug,
                "client_ip": client_ip
            })
            raise HTTPException(
        ...
        if if_none_match and if_none_match == feed_metadata.etag:
            ...
            logger.info("meta_feed_cached", extra={
                "merchant_slug": merchant_slug,
        ...
        headers = {
            ...
            "ETag": feed_metadata.etag,
            "Last-Modified": feed_metadata.last_updated.strftime("%a, %d %b %Y %H:%M:%S GMT"),
        ...
        logger.info("meta_feed_generated_success", extra={
            "merchant_slug": merchant_slug,
        ...
        if "not found" in str(e).lower():
            ...
            logger.warning("merchant_not_found", extra={
                "merchant_slug": merchant_slug,
                "client_ip": client_ip,
                "error": str(e)
            })
            raise HTTPException(
                status_code=404,
                detail=f"Merchant '{merchant_slug}' not found"
            )
        
        # Other validation errors
        increment_counter("meta_feeds_error_total")
        logger.error("meta_feed_validation_error", extra={
            "merchant_slug": merchant_slug,
            "client_ip": client_ip,
            "error": str(e)
        })
        ...
        duration_ms = int((datetime.now() - start_time).total_seconds() * 1000)
        increment_counter("meta_feeds_error_total")
        logger.error("meta_feed_internal_error", extra={
            "merchant_slug": merchant_slug,
            "client_ip": client_ip,
            "error": str(e),
        ...
        raise HTTPException(
            status_code=500,
            detail="Internal server error while generating feed"
        )
...
async def get_feed_stats(
    ...
    **Public Endpoint** - Basic stats for feed monitoring
    ...
    try:
        ...
        logger.info("meta_feed_stats_requested", extra={
            "merchant_slug": merchant_slug,
            "client_ip": client_ip
        })
