...
def validate_logo_file(
    ...
    allowed_types = os.getenv(
        "MEDIA_ALLOWED_TYPES", ",".join(ALLOWED_LOGO_TYPES)
    ).split(",")
    ...
    logger.info(
        f"File validation passed: {normalized_filename}, size: {file_size}, type: {mime_type}"
    )
...
async def upload_logo_to_storage(
    merchant_id: UUID, file: UploadFile, filename: str
) -> Tuple[str, str, int]:
    """
    ...
    try:
        ...
        public_url_result = client.storage.from_(bucket_name).get_public_url(
            storage_path
        )
        ...
        record_histogram(
            "media_upload_size_bytes", file_size, {"merchant_id": str(merchant_id)}
        )
...
async def generate_signed_url(
    merchant_id: UUID, filename: str = "logo.png", expiry_seconds: Optional[int] = None
) -> Tuple[str, datetime]:
    """
    ...
    if expiry_seconds is None:
        expiry_seconds = int(
            os.getenv("SIGNED_URL_EXPIRY", str(DEFAULT_SIGNED_URL_EXPIRY))
        )
    ...
    try:
        ...
        if hasattr(result, "error") and result.error:
            ...
            increment_counter(
                "signed_url_failures_total", {"error": "generation_failed"}
            )
            raise HTTPException(
                status_code=404, detail="Logo not found or access denied"
            )
        ...
        increment_counter(
            "signed_urls_generated_total", {"merchant_id": str(merchant_id)}
        )
...
async def delete_logo_from_storage(
    merchant_id: UUID, filename: str = "logo.png"
) -> bool:
    """
