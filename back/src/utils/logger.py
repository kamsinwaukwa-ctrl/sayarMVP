...
def redact_headers(headers: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in headers.items():
        ...
        elif any(sensitive in key_lower for sensitive in ["token", "secret", "key", "auth"]):
            redacted[key] = REDACT_VALUE
        else:
            redacted[key] = value
    return redacted
...
def redact_query_params(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in params.items():
        key_lower = key.lower()
        if any(sensitive in key_lower for sensitive in ["token", "secret", "key", "auth", "password"]):
            redacted[key] = REDACT_VALUE
        else:
            redacted[key] = value
    return redacted
...
def log_http_response(request_id: str, method: str, path: str, status_code: int, duration_ms: float, **kwargs):
...
def log_auth_event(event_type: str, request_id: str, user_id: Optional[str] = None, merchant_id: Optional[str] = None, **kwargs):
