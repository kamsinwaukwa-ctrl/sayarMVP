...
def sanitize_error_message(message: str) -> str:
    ...
    for pattern in SENSITIVE_PATTERNS:
        sanitized = re.sub(
            pattern, REDACTION_PLACEHOLDER, sanitized, flags=re.IGNORECASE
        )
...
def create_error_response(
    ...
    error_info = ErrorInfo(
        code=code.value, message=sanitized_message, details=details_dict or {}
    )

    return ErrorResponse(error=error_info, timestamp=datetime.now(timezone.utc))


def map_http_exception(
    exc: HTTPException, request_id: Optional[UUID] = None
) -> ErrorResponse:
    """
    ...
    details = None
    if retry_after:
        details = ErrorDetails(reason=detail, retry_after=retry_after)
    else:
...
def map_validation_error(
    exc: ValidationError, request_id: Optional[UUID] = None
) -> ErrorResponse:
    """
    ...
    if len(errors) == 1:
        ...
        details = ErrorDetails(field=field, reason=message, value=error.get("input"))
...
def map_database_error(
    exc: Exception, request_id: Optional[UUID] = None
) -> ErrorResponse:
    """
    ...
    if isinstance(exc, IntegrityError):
        ...
        if isinstance(exc.orig, UniqueViolationError):
            ...
            return create_error_response(
                ErrorCode.CONFLICT, "Unique constraint violation", details, request_id
            )
        elif isinstance(exc.orig, ForeignKeyViolationError):
            details = ErrorDetails(reason="Referenced resource does not exist")
            return create_error_response(
                ErrorCode.VALIDATION_ERROR,
            ...
            details = ErrorDetails(field=column, reason="Field is required")
            return create_error_response(
                ErrorCode.VALIDATION_ERROR,
            ...
            details = ErrorDetails(reason="Data constraint violation")
            return create_error_response(
                ErrorCode.VALIDATION_ERROR,
        ...
        details = ErrorDetails(reason="Invalid data format or type")
        return create_error_response(
            ErrorCode.VALIDATION_ERROR, "Data format error", details, request_id
        )

    # Generic database error
    details = ErrorDetails(reason="Database operation failed")
    return create_error_response(
        ErrorCode.INTERNAL_ERROR, "Database error", details, request_id
    )
...
def map_custom_exception(
    ...
    if isinstance(exc, AuthnError):
        ...
        details = ErrorDetails(reason=exc.reason)
        return create_error_response(
            ErrorCode.AUTHORIZATION_FAILED, str(exc), details, request_id
        )
        ...
        details = ErrorDetails(
            service=exc.service, reason=exc.message, retry_after=exc.retry_after
        )
        return create_error_response(
        ...
        return create_error_response(
            code, f"Upstream service error: {exc.service}", details, request_id
        )

    # Unknown exception
    details = ErrorDetails(reason=f"Unexpected error: {type(exc).__name__}")
    return create_error_response(
        ErrorCode.INTERNAL_ERROR, "Internal server error", details, request_id
    )
...
def handle_business_logic_error(
    operation: str, error_details: Dict[str, Any], request_id: Optional[UUID] = None
) -> ErrorResponse:
    """
    ...
    return create_error_response(
        code, f"Business logic error in {operation}", details, request_id
    )
