...
from ..models.outbox import (
    OutboxEvent, DLQEvent, CreateOutboxEvent, JobType, JobStatus, WorkerHeartbeat
)
...
async def enqueue_job(
    ...
    try:
        ...
        await db.execute(text("SELECT set_config('request.jwt.claims', '{\"role\":\"admin\"}', true)"))
...
async def fetch_due_jobs(
    ...
    try:
        # Set service role for RLS
        await db.execute(text("SELECT set_config('request.jwt.claims', '{\"role\":\"service\"}', true)"))
        ...
        log.error(
            ...
            extra={
                "event_type": "outbox_fetch_failed",
                "error": str(e)
            }
        )
        raise
...
async def move_to_dlq(
    ...
    try:
        ...
        dlq_payload = {
            ...
            "original_payload": json.loads(job_row.payload) if isinstance(job_row.payload, str) else job_row.payload
        }
...
async def get_job_stats(
    ...
    try:
        ...
        log.error(
            ...
            extra={
                "event_type": "outbox_stats_failed",
                "error": str(e)
            }
        )
