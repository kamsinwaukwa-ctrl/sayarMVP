...
from src.integrations.meta_catalog import (
    MetaCatalogClient,
    MetaCatalogConfig,
...
async def handle_wa_send(
    merchant_id: str, payload: Dict[str, Any], wa_rate_limit_per_hour: int = 1000
) -> None:
    ...
    try:
        ...
        logger.info(
            "wa_send_rescheduled",
            extra={"merchant_id": merchant_id, "next_run_at": e.details["reset_time"]},
        )
    ...
    await send_whatsapp_message(
        to=payload["to"], message_type=payload["type"], content=payload["content"]
    )


async def handle_catalog_sync(merchant_id: str, payload: Dict[str, Any]) -> None:
    ...
    try:
        ...
        try:
            ...
            merchant = (
                db.query(Merchant).filter(Merchant.id == UUID(merchant_id)).first()
            )
            if not merchant:
            ...
            meta_credentials = await product_service.load_meta_credentials_for_worker(
                UUID(merchant_id)
            )
            ...
            if sync_payload.action == CatalogSyncAction.UPDATE_IMAGE:
                ...
                image_data = MetaCatalogImageUpdate(
                    image_url=primary_image_url, additional_image_urls=additional_urls
                )
            ...
            if result.success:
                ...
                sync_reason = MetaSyncReasonNormalizer.normalize_errors(
                    result.errors or [], MetaSyncStatus.ERROR.value
                )
            ...
            if result.success:
                ...
                if result.rate_limited:
                    increment_counter(
                        "catalog_sync_rate_limited_total",
                        tags={"merchant_id": merchant_id},
                    )
                else:
                    increment_counter(
                        "catalog_sync_failed_total",
                        tags={
                            "merchant_id": merchant_id,
                            "action": sync_payload.action.value,
                        },
        ...
        logger.error(
            f"Catalog sync error: {str(e)}",
            extra={"merchant_id": merchant_id, "payload": payload, "error": str(e)},
        )
        raise


async def handle_image_cleanup(merchant_id: str, payload: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if success:
            ...
            increment_counter(
                "image_cleanup_success_total",
                tags={"merchant_id": merchant_id, "reason": reason},
            )
        else:
            logger.warning(
...
async def handle_catalog_unpublish(merchant_id: str, payload: Dict[str, Any]) -> None:
    ...
    try:
        ...
        try:
            ...
            result = await meta_client.unpublish_product(
                catalog_id=config.catalog_id, retailer_id=retailer_id, config=config
            )
            ...
            product = (
                db.query(Product)
                .filter(
                    Product.id == product_id, Product.merchant_id == UUID(merchant_id)
                )
                .first()
            )
            ...
            if product:
                if result.success:
                    ...
                    error_message = (
                        "; ".join(result.errors)
                        if result.errors
                        else "Unknown unpublish error"
                    )
                    product.meta_sync_status = "error"
                    product.meta_sync_errors = {
                        "error": error_message,
                        "error_code": "meta_unpublish_failed",
                    }
                    ...
                    increment_counter(
                        "meta_unpublish_errors_total", tags={"error_type": "api_error"}
                    )

                    # Re-raise for retry if it's a retryable error
                    if result.retry_after:
                        raise RetryableError(
                            message=error_message, next_run_at=result.retry_after
                        )
        ...
        increment_counter(
            "meta_unpublish_errors_total", tags={"error_type": "handler_error"}
        )
        raise
