...
class RateLimitConfig(BaseModel):
    ...
    requests_per_minute: float = Field(60.0, ge=0, description="Number of requests allowed per minute")
...
class RateLimitInfo(BaseModel):
    ...
    retry_after: Optional[int] = Field(None, description="Seconds until next request is allowed")
...
class RateLimitResponse(BaseModel):
    ...
    error: Dict[str, Any] = Field(..., description="Error details following BE-007 APIError shape")
...
class MerchantRateLimitConfig(BaseModel):
    ...
    api_rate_limit_per_minute: int = Field(60, ge=0, description="API requests allowed per minute")
    api_burst_limit: int = Field(15, ge=0, description="API burst capacity")
    wa_rate_limit_per_hour: int = Field(1000, ge=0, description="WhatsApp messages per hour")
    rate_limit_enabled: bool = Field(True, description="Whether rate limiting is enabled")
