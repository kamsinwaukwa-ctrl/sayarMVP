...
class SystemSettingCreate(BaseModel):
    ...
    def validate_key_format(cls, v):
        ...
        if not v.replace('_', '').replace('.', '').replace('-', '').isalnum():
            raise ValueError('Key must contain only alphanumeric characters, dots, hyphens, and underscores')
        return v.lower()


class SystemSettingUpdate(BaseModel):
    """Model for updating system settings."""
    value: Optional[Union[str, int, float, bool, Dict[str, Any]]] = Field(None, description="New setting value")
...
class MerchantSettingUpdate(BaseModel):
    ...
    value: Union[str, int, float, bool, Dict[str, Any]] = Field(..., description="New setting value")
...
class FeatureFlagCreate(BaseModel):
    ...
    merchant_id: Optional[UUID] = Field(None, description="Merchant ID for merchant-specific overrides")
    
    @validator('name')
    def validate_name_format(cls, v):
        """Ensure name follows snake_case convention."""
        if not v.replace('_', '').isalnum():
            raise ValueError('Feature flag name must contain only alphanumeric characters and underscores')
        return v.lower()
...
class ConfigurationQuery(BaseModel):
    ...
    key_prefix: Optional[str] = Field(None, min_length=1, description="Filter by key prefix")
...
class BulkConfigurationUpdate(BaseModel):
    ...
    system_settings: Optional[Dict[str, Union[str, int, float, bool, Dict[str, Any]]]] = Field(None, description="System settings to update")
    merchant_settings: Optional[Dict[str, Union[str, int, float, bool, Dict[str, Any]]]] = Field(None, description="Merchant settings to update")
    feature_flags: Optional[Dict[str, bool]] = Field(None, description="Feature flags to update")
...
class MergedConfigurationResponse(BaseModel):
    ...
    system_settings: Dict[str, Union[str, int, float, bool, Dict[str, Any]]] = Field(default_factory=dict)
    merchant_settings: Dict[str, Union[str, int, float, bool, Dict[str, Any]]] = Field(default_factory=dict)
    feature_flags: Dict[str, bool] = Field(default_factory=dict)
    effective_config: Dict[str, Union[str, int, float, bool, Dict[str, Any]]] = Field(default_factory=dict)
...
class ConfigurationHierarchy(BaseModel):
    ...
    source: str = Field(..., description="Source of the effective value (system|merchant)")
