...
class MetaFeedService:
    ...
    async def generate_feed_csv(
        self, merchant_slug: str
    ) -> Tuple[str, MetaFeedResponse]:
        """
    ...
    async def _get_merchant_feed_data(
        self, merchant_slug: str
    ) -> Tuple[Optional[Merchant], List[Product]]:
        """
        ...
        try:
            ...
            logger.error(
                "Failed to get merchant feed data",
                extra={"merchant_slug": merchant_slug, "error": str(e)},
            )
            raise
    ...
    async def _format_products_for_meta(
        ...
        for product in products:
            ...
            availability = (
                "in stock"
                if (product.available_qty and product.available_qty > 0)
                else "out of stock"
            )
            ...
            meta_product = MetaFeedProduct(
                ...
                or f"{self.config.base_url}/images/placeholder.jpg",  # Fallback image
                brand=merchant.name,
    ...
    def _generate_etag(
        self, merchant_slug: str, last_updated: datetime, product_count: int
    ) -> str:
        """
    ...
    async def get_feed_stats(self, merchant_slug: str) -> Optional[MetaFeedStats]:
        ...
        try:
            ...
            in_stock_products = len(
                [p for p in products if p.available_qty and p.available_qty > 0]
            )

            # Get last sync info (would need additional query for actual sync data)
            last_sync_at = (
                max((p.updated_at for p in products), default=None)
                if products
                else None
            )
