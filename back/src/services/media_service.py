...
class MediaService:
    ...
    async def upload_logo(
        ...
        merchant_id: UUID, 
        file: UploadFile,
        update_merchant_record: bool = True
    ) -> MediaUploadResponse:
        """
        ...
        with record_timer("media_upload_duration_seconds", {"merchant_id": str(merchant_id)}):
            try:
                ...
                signed_url, expires_at = await generate_signed_url(merchant_id, normalized_filename)
                
                # Generate storage URL
                storage_url = f"/storage/v1/object/private/merchant-logos/{storage_path}"
                ...
                increment_counter("media_upload_success_total", {"merchant_id": str(merchant_id)})
                logger.info(f"Successfully uploaded logo for merchant {merchant_id}: {normalized_filename}")
                
                return response
                
            except HTTPException:
                increment_counter("media_upload_failed_total", {"merchant_id": str(merchant_id), "error": "validation_error"})
                raise
            except Exception as e:
                logger.error(f"Unexpected error uploading logo for merchant {merchant_id}: {str(e)}")
                increment_counter("media_upload_failed_total", {"merchant_id": str(merchant_id), "error": "unexpected_error"})
    ...
    async def get_logo_signed_url(
        ...
        increment_counter("signed_url_generation_start_total", {"merchant_id": str(merchant_id)})
        ...
        try:
            ...
            if not signed_url:
                ...
                raise HTTPException(status_code=404, detail="No logo found for this merchant")
            ...
            increment_counter("signed_url_generated_total", {"merchant_id": str(merchant_id)})
            logger.info(f"Successfully generated signed URL for merchant {merchant_id}")
            
            return response
            
        except HTTPException:
            increment_counter("signed_url_generation_failed_total", {"merchant_id": str(merchant_id)})
            raise
        except Exception as e:
            logger.error(f"Unexpected error generating signed URL for merchant {merchant_id}: {str(e)}")
            increment_counter("signed_url_generation_failed_total", {"merchant_id": str(merchant_id)})
            raise HTTPException(status_code=500, detail="Internal server error generating signed URL")

    async def delete_logo(self, merchant_id: UUID, update_merchant_record: bool = True) -> bool:
        """
        ...
        increment_counter("media_deletion_start_total", {"merchant_id": str(merchant_id)})
        ...
        try:
            ...
            if deletion_success:
                increment_counter("media_deletion_success_total", {"merchant_id": str(merchant_id)})
            ...
            increment_counter("media_deletion_failed_total", {"merchant_id": str(merchant_id)})
            raise
        except Exception as e:
            logger.error(f"Unexpected error deleting logo for merchant {merchant_id}: {str(e)}")
            increment_counter("media_deletion_failed_total", {"merchant_id": str(merchant_id)})
    ...
    async def _update_merchant_logo_url(self, merchant_id: UUID, logo_url: Optional[str]) -> None:
        """
        ...
        try:
            ...
            logger.warning(f"Failed to update merchant logo_url (this may be expected if migration hasn't been run): {str(e)}")
