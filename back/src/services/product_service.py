...
class ProductService:
        ...
        try:
            ...
            if idempotency_key:
                ...
                if existing_response:
                    logger.info(
                        f"Returning idempotent response for product creation: {idempotency_key}"
                    )
            ...
            brand = self.field_generator.default_brand_from_merchant(
                merchant.name, request.brand
            )

            # 2. Generate SKU if missing or validate existing
            sku = request.sku
            if not sku or not sku.strip():
                sku = await self.field_generator.generate_unique_sku(
                    merchant_id, merchant.slug
                )
                increment_counter(
                    "products_created_with_auto_sku_total",
                    tags={"merchant_id": str(merchant_id)},
                )
            else:
            ...
            if not mpn or not mpn.strip():
                ...
                increment_counter(
                    "products_created_with_auto_mpn_total",
                    tags={"merchant_id": str(merchant_id)},
                )
            ...
            if not request.brand:
                increment_counter(
                    "products_created_with_auto_brand_total",
                    tags={"merchant_id": str(merchant_id)},
                )
                logger.info(
            ...
            # Record metrics
            increment_counter(
                "products_created_total", tags={"merchant_id": str(merchant_id)}
            )
            record_timer(
        ...
        try:
            ...
            if idempotency_key:
                existing_response = await self._check_idempotency(
                    idempotency_key,
                    merchant_id,
                    f"PUT /api/v1/products/{product_id}",
                    request.model_dump(exclude_none=True),
                )
                if existing_response:
                    logger.info(
                        f"Returning idempotent response for product update: {idempotency_key}"
                    )
                    return ProductDB.model_validate(existing_response)
            ...
            if "stock" in update_data:
                update_data["available_qty"] = (
                    update_data["stock"] - product.reserved_qty
                )
            ...
            needs_sync = updated_product.meta_catalog_visible and (
                product.meta_catalog_visible != updated_product.meta_catalog_visible
                or any(
            ...
            if needs_sync:
                ...
                not updated_product.meta_catalog_visible
                and product.meta_catalog_visible
            ):
            ...
            # Record metrics
            increment_counter(
                "products_updated_total", tags={"merchant_id": str(merchant_id)}
            )
            record_timer(
                "product_update_duration_seconds",
                (datetime.now() - start_time).total_seconds(),
            )
    ...
    async def delete_product(
        self, product_id: UUID, merchant_id: UUID, idempotency_key: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            if idempotency_key:
                existing_response = await self._check_idempotency(
                    idempotency_key,
                    merchant_id,
                    f"DELETE /api/v1/products/{product_id}",
                    {},
                )
                if existing_response:
                    logger.info(
                        f"Returning idempotent response for product deletion: {idempotency_key}"
                    )
            ...
            if product.reserved_qty > 0:
                raise ValueError(
                    "Cannot delete product with active inventory reservations"
                )
            ...
            # Record metrics
            increment_counter(
                "products_deleted_total", tags={"merchant_id": str(merchant_id)}
            )
    ...
    async def list_products(
        self, merchant_id: UUID, filters: ProductFilters, pagination: ProductPagination
        ...
        try:
            ...
            if filters.category_path:
                filter_conditions.append(
                    Product.category_path.like(f"{filters.category_path}%")
                )

            if filters.meta_sync_status:
                filter_conditions.append(
                    Product.meta_sync_status == filters.meta_sync_status.value
                )

            if filters.meta_catalog_visible is not None:
                filter_conditions.append(
                    Product.meta_catalog_visible == filters.meta_catalog_visible
                )
            ...
            products = [
                ProductDB.model_validate(row) for row in products_result.fetchall()
            ]
            ...
            logger.error(
                f"Failed to list products: {str(e)}",
                extra={"merchant_id": str(merchant_id), "error": str(e)},
            )
            raise

    async def update_inventory(
        self, product_id: UUID, merchant_id: UUID, stock_delta: int
    ) -> ProductDB:
        ...
        try:
            ...
            update_stmt = (
                ...
                .values(
                    ...
                    updated_at=datetime.now(),
                    meta_sync_status=(
                        MetaSyncStatus.PENDING.value
                        if product.meta_catalog_visible
                        else product.meta_sync_status
                    ),
                )
                .returning(Product)
            )
    ...
    async def enqueue_manual_catalog_sync(
        self, product_id: UUID, merchant_id: UUID, requested_by: UUID
    ) -> str:
        """
        ...
        try:
            ...
            if product_obj.meta_sync_status == MetaSyncStatus.SYNCING.value:
                raise ValueError(
                    "Meta Catalog sync is already in progress for this product"
                )
            ...
            dedupe_key = (
                f"catalog_sync:{merchant_id}:{product_id}:{timestamp.isoformat()}"
            )
            ...
            await self.db.rollback()
            logger.error(
                f"Failed to enqueue manual catalog sync: {str(e)}",
                extra={
                    "merchant_id": str(merchant_id),
                    "product_id": str(product_id),
                    "requested_by": str(requested_by),
    ...
    async def _validate_sku_uniqueness(
        self, merchant_id: UUID, sku: str, exclude_id: Optional[UUID] = None
    ):
        ...
        if result.fetchone():
            increment_counter(
                "sku_duplicate_errors_total", tags={"merchant_id": str(merchant_id)}
            )
    ...
    async def _queue_catalog_sync(self, product_id: UUID, action: str):
        ...
        try:
            ...
            logger.error(
                f"Failed to queue catalog sync: {str(e)}",
                extra={
                    "product_id": str(product_id),
                    "action": action,
                    "error": str(e),
    ...
    async def _check_idempotency(
        self, key: str, merchant_id: UUID, endpoint: str, request_data: Dict[str, Any]
    ...
    async def load_meta_credentials_for_worker(self, merchant_id: UUID):
        ...
        return await self.meta_integration_service.load_credentials_for_worker(
            merchant_id
        )

    async def enqueue_unpublish_on_status_change(
        self, product_id: UUID, merchant_id: UUID, old_status: str, new_status: str
    ) -> None:
        ...
        try:
            ...
            idempotency_key = (
                f"catalog_unpublish:{merchant_id}:{product_id}:status_change"
            )
            ...
            logger.error(
                f"Failed to enqueue unpublish for product {product_id}: {str(e)}"
            )
            increment_counter(
                "meta_unpublish_requests_total",
    ...
    async def enqueue_force_unpublish(
        ...
        try:
            ...
            if product.status in ["archived", "hidden"]:
                raise ValueError(
                    f"Product is already unpublished (status: {product.status})"
                )
            ...
            idempotency_key = (
                f"catalog_unpublish:{merchant_id}:{product_id}:manual:{timestamp}"
            )
            ...
            logger.error(
                f"Failed to enqueue force unpublish for product {product_id}: {str(e)}"
            )
            increment_counter(
                "meta_unpublish_requests_total",
