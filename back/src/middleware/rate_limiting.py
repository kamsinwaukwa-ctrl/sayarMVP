...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            ...
            logger.debug(
                "rate_limit_checked",
                extra={"key": key, "type": "api", "remaining": info.remaining},
            )
            ...
            increment_counter("rate_limit_violations_total", labels={"type": "api"})
            ...
            response = Response(
                status_code=429, content=e.json(), media_type="application/json"
            )
            self._add_rate_limit_headers(response, info)
            return response
...
def create_rate_limit_middleware() -> Callable[[ASGIApp], RateLimitMiddleware]:
    ...
    default_config = RateLimitConfig(
        requests_per_minute=60, burst_limit=15, window_size=60
    )

    # Define stricter limits for auth endpoints
    route_configs = {
        "/api/v1/auth/login": RateLimitConfig(
            requests_per_minute=30, burst_limit=10, window_size=60
        ),
        "/api/v1/auth/register": RateLimitConfig(
            requests_per_minute=10, burst_limit=5, window_size=60
        ),
    }
