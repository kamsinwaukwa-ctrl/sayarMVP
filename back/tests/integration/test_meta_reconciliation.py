...
class TestMetaReconciliationService:
    ...
    async def test_reconciliation_run_lifecycle(
        ...
        with patch.object(service, "_fetch_meta_catalog_data", return_value=meta_items):
            with patch.object(
                service, "_load_meta_credentials", return_value=MagicMock()
            ):
                # Run reconciliation
    ...
    async def test_missing_remote_product(
        self, test_db, sample_merchant, sample_product
    ):
    ...
    async def test_meta_api_error_handling(
        ...
        # Mock Meta API error
        with patch.object(
            service, "_fetch_meta_catalog_data", side_effect=Exception("Meta API error")
        ):
            with patch.object(
                service, "_load_meta_credentials", return_value=MagicMock()
            ):
    ...
    async def test_no_meta_credentials(self, test_db, sample_merchant, sample_product):
        ...
        with patch.object(service, "_load_meta_credentials", return_value=None):
            run = await service.run_reconciliation(
                merchant_id=sample_merchant.id, run_type=ReconciliationRunType.MANUAL
            )
...
class TestReconciliationAdminAPI:
    ...
    async def test_get_reconciliation_status_success(
        self, test_client, test_db, sample_admin, sample_merchant
    ):
    ...
    async def test_trigger_reconciliation_success(
        ...
        with patch(
            "src.api.admin.reconciliation.trigger_manual_reconciliation_for_merchant"
        ) as mock_trigger:
    ...
    async def test_trigger_reconciliation_rate_limited(
        self, test_client, test_db, sample_admin, sample_merchant
    ):
    ...
    async def test_trigger_reconciliation_unauthorized(
        self, test_client, sample_merchant
    ):
    ...
    async def test_get_reconciliation_history(
        self, test_client, test_db, sample_admin, sample_merchant
    ):
    ...
    async def test_get_reconciliation_history_filtered(
        self, test_client, test_db, sample_admin, sample_merchant
    ):
...
class TestMerchantReconciliationAPI:
    ...
    async def test_get_merchant_reconciliation_status(
        self, test_client, test_db, sample_merchant
    ):
    ...
    async def test_get_merchant_reconciliation_status_no_runs(
        self, test_client, sample_merchant
    ):
...
class TestReconciliationWorker:
    ...
    async def test_manual_reconciliation_trigger(
        self, test_db, sample_merchant, sample_product
    ):
        ...
        with patch("src.workers.reconciliation_worker.get_db_session") as mock_get_db:
            ...
            with patch(
                "src.services.meta_reconciliation_service.MetaReconciliationService.run_reconciliation"
            ) as mock_run:
                mock_run.return_value = MagicMock(
                    id=uuid4(), status=MagicMock(value="completed")
                )

                run_id = await trigger_manual_reconciliation_for_merchant(
                    sample_merchant.id
                )
