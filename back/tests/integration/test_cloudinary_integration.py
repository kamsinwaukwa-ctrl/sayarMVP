...
class TestCloudinaryClient:
    ...
    def test_webhook_signature_verification(self, configured_client):
        ...
        assert (
            configured_client.verify_webhook_signature(
...
class TestCloudinaryService:
    ...
    def test_health_check_not_configured(self, mock_health, service):
        ...
        mock_health.side_effect = APIError(
            code=ErrorCode.CLOUDINARY_NOT_CONFIGURED, message="Not configured"
        )
    ...
    def test_delete_product_image_success(self, mock_delete, service, test_product):
        ...
        result = service.delete_product_image(
            image_id=image.id, merchant_id=test_product.merchant_id
        )
        ...
        deleted_image = (
            service.db.query(ProductImage).filter(ProductImage.id == image.id).first()
        )
    ...
    def test_process_webhook_upload_notification(self, service, test_product):
        ...
        with patch.object(
            service.cloudinary_client, "verify_webhook_signature", return_value=True
        ):
...
class TestCloudinaryAPI:
    ...
    def test_health_check_endpoint_success(self, client, auth_headers):
        ...
        with patch(
            "src.services.cloudinary_service.CloudinaryService.health_check"
        ) as mock_health:
            mock_health.return_value = {
                "configured": True,
                "cloud_name": "test-cloud",
            ...
            response = client.get(
                "/api/v1/integrations/cloudinary/health", headers=auth_headers
            )
    ...
    def test_upload_image_endpoint_success(self, client, auth_headers):
        ...
        with patch(
            "src.services.cloudinary_service.CloudinaryService.upload_product_image"
        ) as mock_upload:
    ...
    def test_webhook_endpoint_success(self, client):
        ...
        with patch(
            "src.services.cloudinary_service.CloudinaryService.process_webhook"
        ) as mock_webhook:
...
class TestCloudinaryIntegrationFlow:
    ...
    def test_complete_image_upload_flow(self, setup_test_environment):
        ...
        with patch.object(
            service.cloudinary_client, "upload_image"
            ...
            final_image = (
                ...
                .filter(ProductImage.id == upload_result.id)
                .first()
            )
    ...
    def test_primary_image_switching_flow(self, setup_test_environment):
        ...
        result = service.set_primary_image(
            image_id=image2.id, product_id=product.id, merchant_id=merchant.id
        )
        ...
        updated_image1 = (
            service.db.query(ProductImage).filter(ProductImage.id == image1.id).first()
        )
        updated_image2 = (
            service.db.query(ProductImage).filter(ProductImage.id == image2.id).first()
        )
        ...
        updated_product = (
            service.db.query(Product).filter(Product.id == product.id).first()
        )
