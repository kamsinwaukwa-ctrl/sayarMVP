...
class TestProductsAPI:
    ...
    async def test_create_product_sku_duplicate(
        self, app_client: AsyncClient, test_merchant_jwt: str, test_db: AsyncSession
    ):
    ...
    async def test_get_product(
        self, app_client: AsyncClient, test_merchant_jwt: str, test_product: Product
    ):
    ...
    async def test_delete_product(
        ...
        with patch.object(MetaCatalogClient, "delete_product") as mock_delete:
            mock_delete.return_value = MetaCatalogSyncResult(
                success=True, retailer_id=test_product.retailer_id, sync_duration_ms=100
            )
    ...
    async def test_delete_product_with_reservations(
        self, app_client: AsyncClient, test_merchant_jwt: str, test_db: AsyncSession
    ):
    ...
    async def test_list_products(
        ...
        # Test basic listing
        response = await app_client.get(
            "/api/v1/products", headers={"Authorization": f"Bearer {test_merchant_jwt}"}
        )
        ...
        active_products = [
            p for p in filtered_data["products"] if p["status"] == "active"
        ]
        ...
        visible_products = [
            p for p in visible_data["products"] if p["meta_catalog_visible"]
        ]
    ...
    async def test_update_inventory(
        ...
        assert (
            product["available_qty"] == original_stock + stock_delta
        )  # No reservations
    ...
    async def test_update_inventory_negative_stock(
        self, app_client: AsyncClient, test_merchant_jwt: str, test_product: Product
    ):
    ...
    async def test_meta_sync_error_handling(
        self,
        mock_meta_api,
        app_client: AsyncClient,
        test_merchant_jwt: str,
...
class TestProductService:
    ...
    async def test_sku_uniqueness_validation(self, test_db: AsyncSession):
        ...
        request = CreateProductRequest(
            title="Test Product", price_kobo=15000, stock=100, sku="DUPLICATE-SKU-001"
        )
...
class TestMetaSyncAPI:
    ...
    async def test_trigger_meta_sync_success(
        self, app_client: AsyncClient, admin_token: str, test_db: AsyncSession
    ):
    ...
    async def test_trigger_meta_sync_conflict(
        self, app_client: AsyncClient, admin_token: str, test_db: AsyncSession
    ):
    ...
    async def test_trigger_meta_sync_product_not_found(
    ...
    async def test_trigger_meta_sync_admin_required(
        self, app_client: AsyncClient, test_merchant_jwt: str
    ):
    ...
    async def test_trigger_meta_sync_invalid_uuid(
        self, app_client: AsyncClient, admin_token: str
    ):
    ...
    async def test_enqueue_manual_catalog_sync_service(self, test_db: AsyncSession):
        ...
        await test_db.execute(
            Merchant.__table__.insert().values(
                id=merchant_id, business_name="Test Merchant", slug="test-merchant"
            )
        )
        ...
        with patch("src.services.product_service.enqueue_job") as mock_enqueue:
            ...
            job_id = await service.enqueue_manual_catalog_sync(
                product_id=product_id, merchant_id=merchant_id, requested_by=admin_id
            )
...
class TestMetaUnpublishAPI:
    ...
    async def test_force_unpublish_success(
        self, test_client, test_db, sample_admin, sample_merchant, sample_product
    ):
    ...
    async def test_force_unpublish_not_synced(
        self, test_client, test_db, sample_admin, sample_product
    ):
        ...
        await test_db.execute(
            Product.__table__.update()
            .where(Product.id == sample_product.id)
            .values(meta_sync_status=MetaSyncStatus.NOT_SYNCED.value, retailer_id=None)
        )
        await test_db.commit()
    ...
    async def test_force_unpublish_unauthorized(
        self, test_client, sample_merchant, sample_product
    ):
    ...
    async def test_force_unpublish_already_pending(
        self, test_client, test_db, sample_admin, sample_product
    ):
...
class TestMetaUnpublishJobHandler:
    ...
    async def test_handle_catalog_unpublish_success(
        self, test_db, sample_merchant, sample_product, meta_credentials
    ):
    ...
    async def test_handle_catalog_unpublish_no_credentials(
        self, test_db, sample_merchant, sample_product
    ):
    ...
    async def test_handle_catalog_unpublish_api_failure(
        self, test_db, sample_merchant, sample_product, meta_credentials
    ):
    ...
    async def test_automatic_unpublish_on_status_change(
        self, test_client, test_db, sample_merchant, sample_product
    ):
...
class TestProductAutoGeneration:
    ...
    async def test_create_product_auto_generation_minimal(
        self, app_client: AsyncClient, test_merchant_jwt: str, test_db: AsyncSession
    ):
    ...
    async def test_create_product_explicit_values_preserved(
        self, app_client: AsyncClient, test_merchant_jwt: str, test_db: AsyncSession
    ):
    ...
    async def test_create_product_duplicate_sku_error(
        self, app_client: AsyncClient, test_merchant_jwt: str, test_db: AsyncSession
    ):
    ...
    async def test_update_product_brand_preservation(
        self, app_client: AsyncClient, test_merchant_jwt: str, test_db: AsyncSession
    ):
        ...
        with patch.object(MetaCatalogClient, "create_product") as mock_create:
            mock_create.return_value = MetaCatalogSyncResult(
                success=True, retailer_id="test", meta_product_id="test"
            )
    ...
    async def test_sku_generation_collision_retry(self, test_db: AsyncSession):
        ...
        merchant_id = uuid4()
        merchant = Merchant(
            id=merchant_id, name="Collision Test Merchant", slug="collision-test"
        )
    ...
    async def test_brand_validation_sanitization(self):
        ...
        long_brand = (
            "This is a very long brand name that exceeds the maximum allowed length"
        )
