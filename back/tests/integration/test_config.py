...
from src.models.config import (
    SystemSettingCreate,
    SystemSettingUpdate,
    MerchantSettingCreate,
    MerchantSettingUpdate,
    FeatureFlagCreate,
    FeatureFlagUpdate,
...
class TestSystemSettings:
    ...
    async def test_create_system_setting_admin(
    ...
    async def test_create_system_setting_non_admin_forbidden(
        self, test_client: AsyncClient, merchant_token: str
    ):
        """Test creating system setting as non-admin is forbidden."""
        setting_data = {"key": "test.forbidden.setting", "value": "forbidden_value"}
    ...
    async def test_get_system_setting(
        self, test_client: AsyncClient, admin_token: str, merchant_token: str
    ):
    ...
    async def test_update_system_setting(
        self, test_client: AsyncClient, admin_token: str
    ):
        """Test updating system setting."""
        # Create setting
        setting_data = {"key": "test.update.setting", "value": "original_value"}
    ...
    async def test_list_system_settings(
        self, test_client: AsyncClient, admin_token: str
    ):
    ...
    async def test_delete_system_setting(
        ...
        # Create setting
        setting_data = {"key": "test.delete.setting", "value": "to_be_deleted"}
...
class TestMerchantSettings:
    ...
    async def test_create_merchant_setting(
        self, test_client: AsyncClient, merchant_token: str
    ):
        """Test creating merchant setting."""
        setting_data = {"key": "merchant.test.setting", "value": {"config": "value"}}
    ...
    async def test_merchant_setting_isolation(
        self, test_client: AsyncClient, merchant_token: str, db_session: AsyncSession
    ):
        """Test that merchant settings are properly isolated by RLS."""
        # Create setting for merchant 1
        setting_data = {"key": "isolation.test", "value": "merchant1_value"}
        ...
        setting = await service.get_merchant_setting(
            different_merchant_id, "isolation.test"
        )
        assert setting is None

    async def test_update_merchant_setting(
        self, test_client: AsyncClient, merchant_token: str
    ):
        """Test updating merchant setting."""
        # Create setting
        setting_data = {"key": "merchant.update.test", "value": "original"}
...
class TestFeatureFlags:
    ...
    async def test_create_merchant_feature_flag(
        self, test_client: AsyncClient, merchant_token: str
    ):
    ...
    async def test_feature_flag_hierarchy(
        ...
        merchant_flag = {"name": "hierarchy_test_flag", "enabled": True}
        ...
        hierarchy_flags = [
            f for f in data["data"] if f["name"] == "hierarchy_test_flag"
        ]
        assert len(hierarchy_flags) == 2  # Global + merchant override

        # Merchant-specific flag should be enabled, global should be disabled
        merchant_flag_result = next(
            f for f in hierarchy_flags if f["merchant_id"] is not None
        )
        global_flag_result = next(
            f for f in hierarchy_flags if f["merchant_id"] is None
        )
...
class TestConfigurationHierarchy:
    ...
    async def test_merged_configuration_hierarchy(
        ...
        system_setting = {"key": "hierarchy.test.setting", "value": "system_value"}
        ...
        merchant_setting = {"key": "hierarchy.test.setting", "value": "merchant_value"}
        ...
        assert (
            data["data"]["system_settings"]["hierarchy.test.setting"] == "system_value"
        )

        # Merchant setting should be present
        assert "hierarchy.test.setting" in data["data"]["merchant_settings"]
        assert (
            data["data"]["merchant_settings"]["hierarchy.test.setting"]
            == "merchant_value"
        )
        ...
        assert (
            data["data"]["effective_config"]["hierarchy.test.setting"]
            == "merchant_value"
        )
    ...
    async def test_bulk_configuration_update(
        ...
        bulk_update = {
            ...
            "feature_flags": {"bulk_test_feature": True, "bulk_test_feature2": False},
        }
...
class TestConfigurationCaching:
    ...
    async def test_cache_invalidation(self, test_client: AsyncClient, admin_token: str):
        """Test cache invalidation after updates."""
        # Create initial setting
        setting_data = {"key": "cache.test.setting", "value": "original"}
...
class TestFeatureFlagsEvaluation:
    ...
    async def test_feature_flag_evaluation_service_layer(
...
class TestErrorHandling:
    ...
    async def test_duplicate_system_setting_error(
        self, test_client: AsyncClient, admin_token: str
    ):
        """Test error when creating duplicate system setting."""
        setting_data = {"key": "duplicate.test.key", "value": "first_value"}
    ...
    async def test_invalid_setting_key_format(
        self, test_client: AsyncClient, admin_token: str
    ):
        """Test validation of setting key format."""
        invalid_setting = {"key": "invalid key with spaces!", "value": "test"}
    ...
    async def test_nonexistent_setting_retrieval(
        self, test_client: AsyncClient, merchant_token: str
    ):
    ...
    async def test_unauthorized_system_setting_operations(
        ...
        setting_data = {"key": "unauthorized.test", "value": "test"}
...
class TestConfigurationValidation:
    ...
    async def test_json_value_handling(
        self, test_client: AsyncClient, admin_token: str
    ):
    ...
    async def test_setting_key_normalization(
        self, test_client: AsyncClient, admin_token: str
    ):
        """Test that setting keys are normalized to lowercase."""
        setting_data = {"key": "Test.Mixed.Case.Key", "value": "normalized"}
