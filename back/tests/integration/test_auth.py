...
class TestAuthIntegration:
    ...
    async def test_register_workflow(
        ...
        merchant_result = await db_session.execute(
            text("SELECT * FROM merchants WHERE id = :id"), {"id": merchant["id"]}
        )
    ...
    @pytest.mark.asyncio
    async def test_rls_isolation_after_auth(
        self, app_client: AsyncClient, db_session: AsyncSession
    ):
        ...
        response_2 = await app_client.post(
            "/api/v1/auth/register", json=register_data_2
        )
    ...
    @pytest.mark.asyncio
    async def test_register_without_whatsapp(
        self, app_client: AsyncClient, db_session: AsyncSession
    ):
    ...
    @pytest.mark.asyncio
    async def test_register_with_whatsapp(
        self, app_client: AsyncClient, db_session: AsyncSession
    ):
...
class TestAuthService:
    ...
    async def test_auth_service_login(self, db_session: AsyncSession):
        ...
        login_request = LoginRequest(
            email=register_request.email, password="TestPassword123"
        )
