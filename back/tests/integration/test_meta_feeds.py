...
class TestMetaFeedsAPI:
    ...
    @pytest.mark.asyncio
    async def test_generate_feed_happy_path(self, app_client: AsyncClient, test_merchant_with_products):
        """Test complete feed generation with real products"""
        merchant, products = test_merchant_with_products
        
        # Request feed
        response = await app_client.get(f"/api/v1/meta/feeds/{merchant.slug}/products.csv")
        ...
        assert response.headers["X-Product-Count"] == "2"  # Only visible, active products
        ...
        assert len(rows) == 2  # Only 2 products should be visible (active + meta_catalog_visible)
        ...
        # Verify product data
        face_cream_row = next((row for row in rows if row['title'] == 'Premium Face Cream'), None)
        ...
        lipstick_row = next((row for row in rows if row['title'] == 'Luxury Lipstick'), None)
        assert lipstick_row is not None
        assert lipstick_row['availability'] == 'out of stock'
        assert lipstick_row['inventory'] == '0'
    
    @pytest.mark.asyncio
    async def test_feed_caching_headers(self, app_client: AsyncClient, test_merchant_with_products):
        """Test HTTP caching with ETag and Last-Modified"""
        merchant, _ = test_merchant_with_products
        
        # First request
        response1 = await app_client.get(f"/api/v1/meta/feeds/{merchant.slug}/products.csv")
    ...
    @pytest.mark.asyncio
    async def test_empty_feed_new_merchant(self, app_client: AsyncClient, empty_merchant):
        """Test feed generation for merchant with no products"""
        response = await app_client.get(f"/api/v1/meta/feeds/{empty_merchant.slug}/products.csv")
    ...
    async def test_merchant_not_found(self, app_client: AsyncClient):
        ...
        response = await app_client.get("/api/v1/meta/feeds/nonexistent-merchant/products.csv")
        
        assert response.status_code == 404
        assert "not found" in response.json()["detail"].lower()
    
    @pytest.mark.asyncio
    async def test_rate_limiting(self, app_client: AsyncClient, test_merchant_with_products):
    ...
    @pytest.mark.asyncio
    async def test_feed_stats_endpoint(self, app_client: AsyncClient, test_merchant_with_products):
    ...
    @pytest.mark.asyncio
    async def test_csv_format_compliance(self, app_client: AsyncClient, test_merchant_with_products):
    ...
    @pytest.mark.asyncio
    async def test_feed_url_generation(self, app_client: AsyncClient, test_merchant_with_products):
    ...
    @pytest.mark.asyncio
    async def test_multi_tenant_isolation(self, app_client: AsyncClient, test_db_session):
    ...
    @pytest.mark.asyncio
    async def test_performance_large_catalog(self, app_client: AsyncClient, test_db_session):
