...
class TestOutboxWorkerIntegration:
    ...
    async def test_worker_enqueue_and_process_job(self, test_db):
        ...
        with patch(
            "src.workers.outbox_worker.acquire_leader_lock",
    ...
    async def test_worker_retry_logic(self, test_db):
        ...
        with patch(
            "src.workers.job_handlers.handle_wa_send",
            AsyncMock(
                return_value={
                    "success": False,
    ...
    async def test_worker_concurrent_processing(self, test_db):
        ...
        with patch(
            ...
            worker, "_process_job_with_semaphore", mock_process_with_semaphore
        ):
    ...
    async def test_worker_metrics(self, test_db):
        ...
        initial_count = outbox_jobs_processed_total.labels(
            job_type="wa_send"
        )._value.get()
        ...
        with patch(
            ...
            final_count = outbox_jobs_processed_total.labels(
                job_type="wa_send"
            )._value.get()
