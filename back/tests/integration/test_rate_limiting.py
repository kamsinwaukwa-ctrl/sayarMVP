...
async def test_rate_limit_middleware(client: AsyncClient, merchant_token: str):
    ...
    response = await client.get(
        "/api/v1/merchants/me", headers={"Authorization": f"Bearer {merchant_token}"}
    )
...
async def test_whatsapp_rate_limiting():
    ...
    assert datetime.fromisoformat(exc.value.details["reset_time"]) > datetime.now(
        timezone.utc
    )
...
async def test_wa_worker_rate_limiting(mocker):
    ...
    mocker.patch(
        "src.integrations.whatsapp.WhatsAppIntegration.make_request", return_value={}
    )
...
async def test_rate_limit_api_endpoints(
    client: AsyncClient, admin_token: str, merchant_token: str
):
    """Test rate limit configuration API endpoints."""
    # Test getting own limits
    response = await client.get(
        "/api/v1/rate-limits/me", headers={"Authorization": f"Bearer {merchant_token}"}
    )
...
async def test_concurrent_rate_limiting(client: AsyncClient, merchant_token: str):
