...
class TestMetaCatalogOutboxSync:
    ...
    def test_detect_image_changes_primary_changed(self, catalog_service, product, merchant):
    ...
    def test_detect_image_changes_no_change(self, catalog_service, product, merchant, product_image):
    ...
    def test_enqueue_catalog_sync_success(self, catalog_service, product, merchant, db_session):
        """Test successful catalog sync job enqueuing"""
        changes = {"primary_image_url": "https://res.cloudinary.com/test/image/upload/new.jpg"}
        ...
        sync_log = db_session.query(MetaCatalogSyncLog).filter(
    ...
    def test_enqueue_catalog_sync_idempotency(self, catalog_service, product, merchant, db_session):
        """Test idempotency prevents duplicate jobs"""
        changes = {"primary_image_url": "https://res.cloudinary.com/test/image/upload/same.jpg"}
    ...
    def test_handle_primary_image_change(self, catalog_service, product, merchant, db_session):
        ...
        job_id = catalog_service.handle_primary_image_change(
            product_id=product.id,
            merchant_id=merchant.id,
            new_primary_url=new_url
        )

        assert job_id is not None

        # Verify outbox job created
        outbox_job = db_session.query(OutboxEvent).filter(
    ...
    def test_update_sync_status_success(self, catalog_service, product, merchant, db_session):
        ...
        # Verify update
        updated_log = db_session.query(MetaCatalogSyncLog).filter(
            MetaCatalogSyncLog.id == sync_log.id
        ).first()
        ...
        updated_product = db_session.query(Product).filter(
    ...
    def test_legacy_payload_normalization(self, catalog_service, merchant):
        ...
        legacy_payload = {
            ...
            "changes": {
                ...
                "image_urls": ["https://res.cloudinary.com/test/additional.jpg"]  # Legacy field name
            },
            "idempotency_key": "test_key",
            "triggered_by": "image_upload"
        }

        normalized = catalog_service.normalize_legacy_payload(legacy_payload, merchant.id)
    ...
    @pytest.mark.asyncio
    async def test_handle_catalog_sync_job_update_image(self, db_session, merchant, product):
    ...
    @pytest.mark.asyncio
    async def test_handle_catalog_sync_job_retry_on_rate_limit(self, db_session, merchant, product):
    ...
    def test_cloudinary_service_integration(self, cloudinary_service, product, merchant, db_session):
        """Test Cloudinary service integration with catalog sync"""
        # Mock Cloudinary client
        with patch.object(cloudinary_service.cloudinary_client, 'upload_image') as mock_upload:
            ...
            outbox_jobs = db_session.query(OutboxEvent).filter(
    ...
    def test_sync_metrics_tracking(self, catalog_service, product, merchant, db_session):
    ...
    def test_idempotency_key_generation(self):
        ...
        key1 = MetaCatalogSyncPayload.generate_idempotency_key(product_id, action, content)
        key2 = MetaCatalogSyncPayload.generate_idempotency_key(product_id, action, content)
        ...
        key3 = MetaCatalogSyncPayload.generate_idempotency_key(product_id, action, different_content)

        assert key1 != key3

    def test_legacy_event_normalization_logging(self, catalog_service, merchant, caplog):
    ...
    @pytest.mark.asyncio
    async def test_complete_image_change_to_sync_flow(self, db_session, cloudinary_service, catalog_service, merchant, product):
        ...
        # 2. Process webhook
        with patch.object(cloudinary_service.cloudinary_client, 'verify_webhook_signature', return_value=True):
        ...
        sync_logs = db_session.query(MetaCatalogSyncLog).filter(
            MetaCatalogSyncLog.merchant_id == merchant.id,
            MetaCatalogSyncLog.product_id == product.id
        ).all()
        ...
        updated_sync = db_session.query(MetaCatalogSyncLog).filter(
            MetaCatalogSyncLog.id == latest_sync.id
        ).first()
