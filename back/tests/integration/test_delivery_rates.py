...
class TestDeliveryRatesIntegration:
    ...
    @pytest.mark.asyncio
    async def test_create_delivery_rate_workflow(
        self, app_client: AsyncClient, admin_user_token: str, db_session: AsyncSession
    ):
        ...
        rate_result = await db_session.execute(
            text("SELECT * FROM delivery_rates WHERE id = :id"), {"id": rate["id"]}
        )
    ...
    @pytest.mark.asyncio
    async def test_list_delivery_rates_workflow(
        self, app_client: AsyncClient, admin_user_token: str, delivery_rates_fixture
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_delivery_rate_workflow(
        self,
        app_client: AsyncClient,
        admin_user_token: str,
        delivery_rates_fixture,
        ...
        rate_result = await db_session.execute(
            text("SELECT * FROM delivery_rates WHERE id = :id"), {"id": active_rate_id}
        )
        db_rate = rate_result.fetchone()
    ...
    @pytest.mark.asyncio
    async def test_update_rate_business_validation(
        self,
        app_client: AsyncClient,
        admin_user_token: str,
        single_active_rate_fixture,
        db_session: AsyncSession,
    ):
        ...
        rate_result = await db_session.execute(
            text("SELECT active FROM delivery_rates WHERE id = :id"), {"id": rate_id}
        )
        db_rate = rate_result.fetchone()
        assert db_rate.active is True

    @pytest.mark.asyncio
    async def test_delete_delivery_rate_workflow(
        self,
        app_client: AsyncClient,
        admin_user_token: str,
        delivery_rates_fixture,
    ...
    @pytest.mark.asyncio
    async def test_delete_rate_business_validation(
        self,
        app_client: AsyncClient,
        admin_user_token: str,
        single_active_rate_fixture,
        ...
        rate_result = await db_session.execute(
            text("SELECT * FROM delivery_rates WHERE id = :id"), {"id": rate_id}
        )
        db_rate = rate_result.fetchone()
        assert db_rate is not None
        assert db_rate.active is True
    ...
    @pytest.mark.asyncio
    async def test_staff_user_access_control(
        self, app_client: AsyncClient, staff_user_token: str
    ):
        ...
        rate_data = {"name": "Test Rate", "areas_text": "Test Area", "price_kobo": 1000}
    ...
    async def test_unauthorized_access(self, app_client: AsyncClient):
        ...
        response = await app_client.get(
            "/api/v1/delivery-rates", headers={"Authorization": "Bearer invalid_token"}
        )
        assert response.status_code == 401
    ...
    async def test_cross_tenant_isolation(
        ...
        app_client: AsyncClient,
        admin_user_token: str,
        other_merchant_admin_token: str,
        delivery_rates_fixture,
    ):
    ...
    async def test_validation_errors(
        self, app_client: AsyncClient, admin_user_token: str
    ):
        ...
        invalid_update = {"price_kobo": -500}  # Negative price
...
async def delivery_rates_fixture(db_session: AsyncSession, test_merchant_id: str):
    ...
    return {"active_rate_id": active_rate_id, "inactive_rate_id": inactive_rate_id}
